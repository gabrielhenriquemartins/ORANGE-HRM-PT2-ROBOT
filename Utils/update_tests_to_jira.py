...
def get_values_in_command_line():
    global project_key, test_plan, JIRA, API_TOKEN, USER
    if len(sys.argv) == 6:
...
def create_test_and_return_id(project_key, TEST_NAME):
    ...
    jql = f'project = {project_key} AND summary ~ "\\"{TEST_NAME}\\"" AND summary ~ "Robot" AND issuetype = Test'
    params = {
        'jql': jql,
        'fields': 'key'
    }
                        ...
                        for index, line in enumerate(file):
                            if control_variavel == 1:
                                ...
                                if TEST_NAME in line:
                                    modified_line = line.replace(TEST_NAME, f'[Robot] {TEST_NAME}')
                                    modified_lines.append(modified_line)
                                    control_variavel = 1
                                else:
